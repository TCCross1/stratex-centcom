...
const job = await RealityEngine.queueJob({ propertyId: "SXP-004182", twinType: "TWIN_TYPE_A", processorType: "MESH_GENERATION" });
check(job.status === "QUEUED" && job.sourceMode === "FIXTURE", "a queued job is marked as a fixture job");
const ran = await RealityEngine.runJob(job.jobId);
